...
**字谜**字符串是由同一组字符组成的两个字符串，其中字符的顺序是字符串中唯一的区别。比如**【沉默】****【听】**是字谜，或者**【偷看】****【保持】**是字谜。

## 如何用 PHP 检查这两个字符串是不是字谜？

在这个程序中，我们接受来自用户的字符串，并检查这些字符串是否是字谜。首先，我们必须对字符串进行排序，因为我们必须将字符串转换成数组，然后将它们转换回字符串，并检查这些字符串，如果两者相同，则它们是字谜，否则它们不是字谜字符串。对于这些过程，我们使用某些内置函数
...
**第三步:**检查情况**strlen(S1)！= strlen(s2)'** 如果为真，打印输入的字符串不是字谜，退出程序，否则执行以下步骤
...
**步骤 8:** 检查条件**‘strcmp(S1，S2)= = 0’**如果为真，则打印输入的字符串是字谜，否则打印输入的字符串不是字谜
