...
> 原文:[https://learnetutorials.com/c-programming/strings](https://learnetutorials.com/c-programming/strings)
...
这里`%c`是用于逐个打印字符串的格式说明符，而`%s`是用于打印字符串文字的格式说明符。
...
C 语言中有一套 I/O 功能，用于访问键盘输入，并按要求显示在屏幕上。常用的输出函数有:printf()，put()，putchar()，输入函数有 scanf()，get()，getchar()，getch()，getche()。
